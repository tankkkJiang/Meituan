            ...
            for it in iterators[:]:
                while True:  # 在每个迭代器中使用 while 循环
                    try:
                        ...
                        if self.waybill_count_start > 1 and (select_start_time_ms < (waybill['orderTime'] + 3000) or select_start_time_ms > (waybill['timeout'] - 240000) or select_start_time_ms > (waybill['betterTime'] - 150000) ):
                            # 丢弃这一单，直接开始下一单
                            self.loss_waybill += 1
