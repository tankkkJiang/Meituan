            ...
            for it in iterators[:]:
                while True:  # 在每个迭代器中使用 while 循环
                    ...
                    try:
                        ...
                        if self.waybill_count_start > 1 and (select_start_time_ms > (waybill['orderTime'] + 20000)) and ((select_start_time_ms + 15000 > (waybill['timeout'])) or (select_start_time_ms + 135000 > ((waybill['betterTime'] + waybill['timeout']))/4)):
                            # 丢弃这一单，直接开始下一单
