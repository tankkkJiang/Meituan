...
class DemoPipeline:
    ...
    def judge_fly_or_not(self):
        ...
        for i in range(len(self.route_state)):
            ...
            if cur_route_state.cur_drone_cnt < cur_route_state.max_drone_cnt:
                ...
                # 计算需要的最小时间差为多少，
                '''**最小时间差需要微调 '''
                min_time = (2 * cur_route_state.height + 10) - 40  # 上升和降落的距离再加10
    ...
    def set_back_drone_route(self, car_sn, drone_sn, route, route_id):
        ...
        if route_id == 0:
            ...
            route[0].y = drone_pos.y  # 将返航的第一个中间点向y轴方向，拉斜，以避免一些极端的情况
        elif route_id == 1:
    ...
    day 2024 10.5 8:00
    看日志发现目前有三个问题（是否还有不确定）：
    (1) 航线交叉问题：
        SIM-DRONE-6 从卸货点2{Position: x=508, y=522.01, z=-81}请求返航到位置{Position: x=185.12, y=432.89, z=-21}(SIM-MAGV-1的位置)
                    返航路线为[Position(0, 0, -84), Position(0, 0, -84), Position(0, 0, -21)]
        SIM-DRONE-3 也在从卸货点1{Position: x=146, y=194, z=-84}请求返航到位置{Position: x=190.04, y=435.85, z=-21}(SIM-MAGV-4的位置)
                    返航路线为[Position(0, 0, -84), Position(210, 260, -84), Position(0, 0, -84), Position(0, 0, -21)]
        错误发生的位置为  errorposition{\"x\":189.98,\"y\":434.18,\"z\":-84.19}

    (2) 返航请求不及时的问题：
        SIM-DRONE-8 起飞到卸货点2{Position: x=508, y=514, z=-27}的过程中
                    前往路线为[Position(0, 0, -74), Position(490, 500, -74), Position(508, 514, -74), Position(508, 514, -27)]
        SIM-DRONE-4 在卸货点2长时间等待，然后终于等到空车了，然后返航到位置{Position: x=194.93, y=432.97, z=-21}(SIM-MAGV-5的位置)
                    返航路线为[Position(0, 0, -84), Position(0, 0, -84), Position(0, 0, -21)]
        然后好巧不巧，这时候8也开始降落了，他俩就面对面相拥了，😮‍💨，难受
        错误发生的位置为 errorPosition\":{\"x\":508.0,\"y\":518.43,\"z\":-55.56}

    (3) 不明所以的问题ERROR_EVENT_DRONE_LANDING_NOT_AVIATION_OPERATION_POINT
        空


    (4) 订单分类少了个if -1 导致部分订单混乱
                ...
                if (self.des_pos_reached(cur_drone.des_pos, cur_drone_pos, 3)
                    ...
                    for j in range(len(self.car_list) - 1, -1, -1):  # 逆序遍历空闲无人车，这样后面的就先停下了
                        ...
                        if cur_car.state == "empty":
                            ...
                            # 设置返回航线，再写个函数set_back_drone_route()
